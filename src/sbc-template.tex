...
% Proposal
Nossa proposta de ferramenta contempla a visualização da ruído de um controle do óculos de realidade virtual da Samsung e um componente implementado em \textit{Unity3D} para captura e filtragem do posicionamento do cursor da aplicação.

A figura \ref{fig:system_blocks} apresenta a arquitetura da nossa implementação. Primeiramente, capturamos a orientação angular do controle fornecida pela \textit{OVRInput API}. O próximo passo é estimar a posição do cursor dado a angulação do controle através da intersecção de um raio com a direção do controle e uma esfera de tamanho unitário. Este ponto de intersecção é a posição que o usuário está tentando apontar com o controle. Nessa posição é aplicado o filtro de Kalman para estabilização e por fim são renderizadas as posições ruidosas e o caminho percorrido pela posição estabilizada.
...
A figura \ref{fig:chart_noise} mostra o gráfico da velocidade do curso ao longo da execução da aplicação. Ao comparar com o gráfico gerado na mesma execução mas com o cursor filtrado, verificamos que a aplicação foi capaz de filtrar o posicionamento capturado pelo controle.
