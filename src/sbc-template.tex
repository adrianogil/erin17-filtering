...
Fundamentalmente pode-se definir, filtros analógicos como aqueles que fazem uso de componentes eletrônicos, tais como resistores e capacitores, enquanto os digitais operam sobre informações computadorizadas por meio de cálculos numéricos.

O filtro de passa-baixa (\textit{low-pass}) e passa-alta (\textit{high-pass}) exemplificam de maneira intuitiva a funcionalidade de um filtro. Os filtros \textit{low-pass} permitem que as frequências abaixo da frequência de corte sejam consideradas à medida que os \textit{filtros high-pass} permitem a passagem das frequências acima da base de corte.
...
Cabe ainda esclarecer o conceito de filtro de Kalman como uma função de probabilidade que descreve flutuações randômicas num espaço contínuo. Essencialmente, o filtro de Kalman soluciona o problema de tentar estimar o estado $  x \in \mathbb{R}^{n}$ pela equação de diferença estocástica linear:
...
A primeira equação representa que cada $x_k$ é a combinação linear do valor anterior somado ao controle de sinal $_k$ e um ruído de processo. A segunda equação mostra que qualquer valor medido é a combinação linear do valor do sinal $x_k$ a uma medida de ruído do medição.

O filtro aplica ainda dois conjuntos de equações, organizados em dois passos: termos de predição e termo de correção, ambos de natureza recursiva, isto é, há um passo de previsão e um de correção das medidas obtidas. Abaixo seguem os conjuntos de equações, bem como esclarecimentos a respeito das mesmos, consoante \cite{WelchBishop} e \cite{LAARAIEDH}.
...
Durante a execução da aplicação, percebemos que quando a movimentação do cursor é muito mais rápida que a taxa de atualização, i.e., seu \textit{Update}, verificamos que os pontos de ruído não são visualizados de maneira contínua, provocando o efeito visto na figura \ref{fig:filter02}, onde as partículas se concentram em apenas alguns pontos amostrados pelo \textit{framerate} da aplicação \textit{Unity}, gerando regiões ruidosas espaçadas entre si.
