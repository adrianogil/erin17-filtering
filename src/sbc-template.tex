...
O filtro de passa-baixa (low-pass) e passa-alta (high-pass) exemplificam de maneira intuitiva a funcionalidade de um filtro. Os filtros \textit{ low-pass} permitem que as frequências abaixo da frequência de corte sejam consideradas à medida que os \textit{filtros high-pass} permitem a passagem das frequências acima da base de corte. As ilustrações abaixo permitem visualizar o comportamento esperado:
...
Cabe ainda esclarecer um conceitos anteriormente mencionado, ruído gaussiano, antes de seguir-se adiante. Trata-se de uma função de probabilidade que descreve flutuações randômicas num espaço continuo.
...
O filtro aplica ainda dois conjuntos de equações, organizados em dois passos: \textit{prediction} e \textit{correction} de natureza recursiva, isto é, há um passo de previsão e um de correção das medidas obtidas. Abaixo seguem os conjuntos de equações, bem como esclarecimentos a respeito das mesmas, consoante \cite{WelchBishop} e \cite{LAARAIEDH}.
...
% * Tipos de Controles
Existem vários tipos de controles, o próprio headset, i.e., o equipamento de realidade virtual vestível também possui um controle baseado no giroscópio, também existem outros tipos de controle que além disso fornecem informações posicionais, adicionando um grau de liberdade a mais para o usuário, que pode manipular em diferentes graus na profundidade do espaço virtual.


% * Descrever controle da Samsung
A figura \ref{fig:vrcontroller} mostra o controle que acompanha o óculus de realidade virtual \textit{GearVR SM-R324} da Samsung. A \textit{Oculus} é atualmente parceira da Samsung na distribuição de soluções de realidade virtual utilizando dispositivos móveis e disponibiliza um \textit{SDK} para uso de funcionalidades específicas de aplicações de realidade virtual, dentre elas uma API para captação dos dados de rotação do controle \cite{gearvrinputdocs}.
...
% Proposal
A proposta deste artigo é a implementação de uma ferramenta para visualizar a aplicação do filtro de Kalman na estabilização de um controle para realidade virtual. O uso do filtro é motivado pela necessidade de remoção de ruído e obtenção de um controle mais estável para o usuário. Nossa contribuição para o desenvolvedor é a capacidade de visualizar o efeito do ruído e entender como este é amenizado pelo filtro de Kalman, e para o usuário, a criação de um componente para controle de realidade virtual que possibilite movimentos mais precisos.


% As a didatic tool
Dado o objetivo de remover ruído de um determinando conjunto de dados sinal, é de grande interesse ao desenvolver conseguir entender como esse ruído afeta a aplicação de realidade virtual, seja produzindo um cursor mais trêmulo ou dificultando a interação do usuário. 


% Describe Unity implementation - Noise visualization
Para visualizar o ruído, utilizamos elementos de partículas ou \textit{ParticleSystem} para renderizar os pontos onde o cursor deveria estar levando em conta as entradas ruidosas tais como recebidas pelo controle. As partículas geradas foram configuradas para ter um cor destoante, uma tonalidade de vermelho indica a entrada real obtida pelo controle, e ao longo do tempo reduz-se a luminosidade das partículas de modo a adicionar uma informação temporal a visualização gerada. Assim, pontos vermelhos mais vivos indicam os dados mais recentes recebidos pelo controle enquanto pontos mais fracos representam posições mais antigas.
...
Nossa implementação em Unity foi executada em um aparelho Samsung S8 acoplado no óculos \textit{GearVR SM-R324}. Foi utilizado o controle oficial da Samsung que acompanha o óculos de realidade virtual \textit{GearVR}. Para fins de teste, foi gerada uma esfera invertida e um código em \textit{shader} para exibir grids na superfíce da esfera gerando um ambiente branco de cor sólida e sem efeitos de iluminação.

Toda aplicação \textit{GearVR} distribuída na \textit{Oculus Store} \cite{gearvrstore} pode ser executada nos modelos de celulares Samsung mais avançados e acoplado em um óculos \textit{GearVR}. A qualquer momento durante a execução da aplicação é possível acessar um meno de opções que permite registrar \textit{screenshots} ou gravar vídeos da experiência do usuário dentro do mundo virtual da aplicação.

A figura \ref{fig:filter01} mostra um \textit{screenshot} da aplicação em Realidade Virtual com a filtragem de cursor ativa. O caminho do cursor é definido pela rotação atual do controler do \textit{GearVR}. A posição do cursor é calculada na intercecção de uma esfera de tamanho unitário e a direção atual do controle.
